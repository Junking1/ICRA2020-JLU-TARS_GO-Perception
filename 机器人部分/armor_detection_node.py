...
class ArmorDetectionNode():
	...
	def __init__(self):
		...
		self._direction_pub = rospy.Publisher('/enemy_direc',EnemyDirection,queue_size=1, tcp_nodelay=True)
		...
		while not rospy.is_shutdown():
			...
			if self._can_ctrl:
				if angle is not None:
					...
					self._shoot(1,1)
					rospy.loginfo('pub angle: pitch '+str(angle[0])+' yaw '+str(angle[1]))
				elif undet_count != 0:
	...
	def calcu_angle(self, bbox):
		if bbox[2] == 0:
			...
			box = [bbox[1], bbox[0], bbox[1]+bbox[3], bbox[0]+bbox[2]] # [ymin xmin ymax xmax]
			object_2d_point = np.array(([box[1],box[0]],[box[1],box[2]],[box[3],box[2]],[box[3],box[0]]),dtype=np.double)
			_, rvec, tvec = cv2.solvePnP(self.object_3d_points, object_2d_point, self.camera_matrix, self.dist_coefs, cv2.SOLVEPNP_EPNP)
...
if __name__ == '__main__':
	...
	detector = Detector('detector', detecting, tracking, initracker, boundingbox, direction, image_in, ENEMY_COLOR)
	tracker  = Tracker ('tracker', detecting, tracking, initracker, boundingbox, flag, image_in)
	detector.start()
	tracker.start()
